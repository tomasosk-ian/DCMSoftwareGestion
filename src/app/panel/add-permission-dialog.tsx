...
export function AddPermissionDialog() {
  const { mutateAsync: createPermission, isLoading } =
    api.permissions.create.useMutation();

  const [access, setAccess] = useState<string[]>([]);
  const [description, setDescription] = useState("");
  const [type, setType] = useState("");
  const [types, setTypes] = useState(["Rutas", "UX", "Otro"]);
    ...
    <>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="sm:max-w-[425px]">
          ...
          <div>
            <Label>El permiso se editará al crearse.</Label>
          </div>
          {/* <div>
